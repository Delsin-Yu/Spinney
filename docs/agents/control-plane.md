...
| `GET /state` | window `busy` + active session/node + the session list; each session carries `id`, `title`, `nodes`, `active`, `titleSource`, `titleLocked`, `running`, `runningNodes`, `runningBackgrounds`, `backgroundNodes`, `model`, `effort` |
