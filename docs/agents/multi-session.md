...
| message | shape | notes |
| --- | --- | --- |
| `state` | `{ sessionId, busy, status, runningNodes: string[], lockedNodes: string[] }` | `busy` = any run in the session; `runningNodes` = nodes with a live run. Composer shows Stop iff `runningNodes.includes(viewFocusId)`. `lockedNodes` = nodes that are *not* streaming but own unfinished work (a running background job / async sub-agent batch, or a notice about to be injected into them): the composer disables input / Send / attach there and the host refuses the send (`onUserMessage`, `/continue`, `/session/start`). Only the owner is listed — its existing descendants stay usable. |
