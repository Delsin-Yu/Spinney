 ...
 *   1. media/tree.js must reproduce the benchmarked candidate designN
 *      (non-layered-tidy-tree-layout with engine-owned sidecar reservation)
 *      position-for-position,
 *   2. and it must have zero overlaps, zero direction violations, zero
 *      interpositions and zero connector crossings on every scenario.
 *
 *   node verify-tree.js
 */
const fs = require('fs');
...
for (const sc of scenarios()) {
  ...
  const b = N.layoutEngineReserve(t.nodesById, 'n0', t.heights, o);
  ...
  // sidecar windows (it runs inside the parent's reserved sidecar area). Only a
  // FOREIGN card in the connector's path is a defect.
  ...
  for (const pid in t.nodesById) {
    ...
    for (const x of ak) for (const d of require('./violations').descendants(t.nodesById, x)) ownGroup.add(d);
  }
  ...
  console.log(
    `${ok ? 'OK  ' : 'FAIL'} ${sc.name.padEnd(8)} n=${String(sc.n).padStart(3)} ` +
      ...
      `interpose=${v.interpositions.length} crossForeign=${foreignCross.length} crossOwnGroup=${v.crossings.length - foreignCross.length} ` +
