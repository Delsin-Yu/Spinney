...
- `vscode:prepublish` (compile plus the five guards) is the release gate: a release does not ship when that script fails.
...
- Model and thinking effort are per session (one per `SessionRuntime`; the default comes from the global record / setting). A model is a **model card** — a row of `spinney.modelCards` bound to a `spinney.providers` endpoint, edited by the `Spinney: Model Cards` page, not by the Settings UI — and a session's level is clamped to that card's own `efforts` (`docs/agents/invariants/model-cards.md`).
